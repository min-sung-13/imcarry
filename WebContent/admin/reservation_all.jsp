...
<c:set var="bookList" value="${bookDAO.selectBookList(limit)}" />
<c:set var="page" value="${util.paging(bookDAO.bookListSize(), 15)}" />
<c:if test="${param.keyword ne null}">
	<c:set var="bookList" value="${bookDAO.selectBookList(limit, param.keyword)}" />
</c:if>
<div class="col-lg-6 col-md-12">
	<div class="card">
		<div class="card-header card-header-warning all">
			<h4 class="card-title">모든 예약</h4>
		...
		<div class="card-body table-responsive">
			<div class="minisearch_form_wrap">
				<form method="post" action="./reservation.jsp" class="minisrch_form">
					...
					<input type="button" value="전체보기" id="reset_btn_a">
					<input type="hidden" name="btn" value="all" >
				</form>
			</div>
				...
				<tbody>
					...
					<c:forEach var="book" items="${bookList}" varStatus="status">
						<tr>
							...
							<c:choose>
								<c:when test="${book.bEnddate > util.getToday()}" >
			...
			<div class="page_wrap">
				<ul class="page_list">
					<c:forEach var="i" items="${page}" varStatus="status" >
						<li><a href="./reservation.jsp?btn=all&&pageNum=${status.index + 1}"<c:if test="${status.index + 1 eq pageNum}">class="on"</c:if>>${status.index + 1}</a></li>
					</c:forEach>
			    </ul>
			</div>
		</div>
